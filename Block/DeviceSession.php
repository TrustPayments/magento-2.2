 ...
 */
namespace TrustPayments\Payment\Block;
...
class DeviceSession extends \Magento\Framework\View\Element\Template
{
    ...
    {
        return $this->getUrl('trustpayments_payment/checkout/deviceSession', ['_secure' => $this->getRequest()->isSecure()]);
    }
    ...
    {
        $device = $this->scopeConfig->getValue('trustpayments_payment/checkout/fingerprint', ScopeInterface::SCOPE_STORE, $this->_storeManager->getStore());
        if ($device!=1) {
            return false;
        }

        $baseUrl = \rtrim($this->scopeConfig->getValue('trustpayments_payment/general/base_gateway_url'), '/');
