 ...
 */
namespace TrustPayments\Payment\Helper;
...
class Document extends AbstractHelper
{
    ...
    public function isInvoiceDownloadAllowed(TransactionInfo $transaction, $storeId = null)
    {
        ...
        if (! $this->helper->isAdminArea() && ! $this->scopeConfig->getValue(
            'trustpayments_payment/document/customer_download_invoice', ScopeInterface::SCOPE_STORE, $storeId)) {
            return false;
        }
    ...
    public function isPackingSlipDownloadAllowed(TransactionInfo $transaction, $storeId = null)
    {
        ...
        if (! $this->helper->isAdminArea() && ! $this->scopeConfig->getValue(
            'trustpayments_payment/document/customer_download_packing_slip', ScopeInterface::SCOPE_STORE,
            $storeId)) {
            return false;
        }
    ...
    {
        if (! $this->helper->isAdminArea() && ! $this->scopeConfig->getValue(
            'trustpayments_payment/document/customer_download_refund', ScopeInterface::SCOPE_STORE, $storeId)) {
            return false;
        }
