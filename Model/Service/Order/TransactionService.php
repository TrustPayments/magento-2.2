 ...
 */
namespace TrustPayments\Payment\Model\Service\Order;
...
class TransactionService extends AbstractTransactionService
{
    ...
    protected function collectMetaData(Order $order)
    {
        ...
        $this->eventManager->dispatch('trustpayments_payment_collect_meta_data',
            [
                'transport' => $transport,
                'order' => $order
            ]);
    ...
    public function accept(Order $order)
    {
        return $this->apiClient->getService(DeliveryIndicationService::class)->markAsSuitable(
            $order->getTrustpaymentsSpaceId(), $this->getDeliveryIndication($order)
                ->getId());
    }
    ...
    protected function getDeliveryIndication(Order $order)
    {
        ...
        $query->setFilter(
            $this->helper->createEntityFilter('transaction.id', $order->getTrustpaymentsTransactionId()));
        $query->setNumberOfEntities(1);
        return \current(
            $this->apiClient->getService(DeliveryIndicationService::class)->search(
                $order->getTrustpaymentsSpaceId(), $query));
    }
