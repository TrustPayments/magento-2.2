...
abstract class AbstractTransactionService
{
     ...
     *
     * @param \TrustPayments\Sdk\Model\PaymentMethodConfiguration[] $paymentMethods
     */
    ...
    protected function updateQuote(Quote $quote, Transaction $transaction)
    {
        $this->resource->getConnection()->update($this->resource->getTableName('quote'),
            [
                'trustpayments_space_id' => $transaction->getLinkedSpaceId(),
                'trustpayments_transaction_id' => $transaction->getId()
            ], [
    ...
    {
        return $this->cookieManager->getCookie('trustpayments_device_id');
    }
