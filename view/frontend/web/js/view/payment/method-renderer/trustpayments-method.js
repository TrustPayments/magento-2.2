...
define([
	...
	'TrustPayments_Payment/js/model/checkout-handler'
], function(
	$,
	Component,
	fullScreenLoader,
	...
	return Component.extend({
		defaults: {
			template: 'TrustPayments_Payment/payment/form'
		},
		...
		initialize: function(){
			...
			if (window.checkoutConfig.trustpayments.integrationMethod == 'iframe') {
		...
		validateTrustpayments: function(){
			if (window.checkoutConfig.trustpayments.integrationMethod == 'iframe') {
                ...
                this.getPlaceOrderDeferredObject()
                    .fail(
                        function (response) {
                        	...
                        	if (typeof error == 'object' && error.message == 'trustpayments_checkout_failure') {
                        		window.location.replace(urlBuilder.build("trustpayments_payment/checkout/failure"));
                        	} else {
		...
		afterPlaceOrder: function(){
			...
			window.history.pushState({}, document.title, window.checkoutConfig.trustpayments.restoreCartUrl);
			
			fullScreenLoader.startLoader();
			
			if (window.checkoutConfig.trustpayments.integrationMethod == 'iframe' && this.handler) {
				this.handler.submit();
			} else if (window.checkoutConfig.trustpayments.integrationMethod == 'lightbox' && typeof window.LightboxCheckoutHandler != 'undefined') {
		...
		fallbackToPaymentPage: function(){
			fullScreenLoader.startLoader();
			if (window.checkoutConfig.trustpayments.paymentPageUrl) {
				window.location.replace(window.checkoutConfig.trustpayments.paymentPageUrl + "&paymentMethodConfigurationId=" + this.getConfigurationId());
			} else {
				window.location.replace(urlBuilder.build("trustpayments_payment/checkout/failure"));
			}
		},
