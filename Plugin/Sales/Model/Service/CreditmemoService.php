 ...
 */
namespace TrustPayments\Payment\Plugin\Sales\Model\Service;
...
use TrustPayments\Payment\Model\Payment\Method\Adapter as PaymentMethodAdapter;
use TrustPayments\Payment\Model\Service\LineItemReductionService;
use TrustPayments\Payment\Model\Service\RefundService;
use TrustPayments\Sdk\Service\RefundService as ApiRefundService;
...
class CreditmemoService
{
        ...
        \Magento\Sales\Api\Data\CreditmemoInterface $creditmemo, $offlineRequested = false)
    {
        try {
            ...
            if ($creditmemo->getTrustpaymentsKeepRefundJob() !== true) {
                try {
        ...
        \Magento\Sales\Api\Data\CreditmemoInterface $creditmemo, $offlineRequested = false)
    {
        ...
        if ($creditmemo->getOrder()
            ...
            $creditmemo->getTrustpaymentsExternalId() == null) {
            try {
        ...
        try {
            ...
            try {
                $this->apiClient->getService(ApiRefundService::class)->refund(
                    $order->getTrustpaymentsSpaceId(), $existingRefundJob->getRefund());
            } catch (\Exception $e) {
