...
class InstallSchema implements InstallSchemaInterface
{
    ...
    {
        $installer->getConnection()->addColumn($installer->getTable('quote'), 'trustpayments_space_id',
            [
                'type' => Table::TYPE_BIGINT,
                'unsigned' => true,
                'comment' => 'Trust Payments Payment Space Id'
            ]);

        $installer->getConnection()->addColumn($installer->getTable('quote'), 'trustpayments_transaction_id',
            [
                'type' => Table::TYPE_BIGINT,
                'unsigned' => true,
                'comment' => 'Trust Payments Payment Transaction Id'
            ]);
    ...
    {
        $installer->getConnection()->addColumn($installer->getTable('sales_order'), 'trustpayments_space_id',
            [
                'type' => Table::TYPE_BIGINT,
                'unsigned' => true,
                'comment' => 'Trust Payments Payment Space Id'
        ...
        $installer->getConnection()->addColumn($installer->getTable('sales_order'), 'trustpayments_authorized',
            [
                'type' => Table::TYPE_BOOLEAN,
                'default' => false,
                'comment' => 'Trust Payments Payment Authorized'
            ]);

        $installer->getConnection()->addColumn($installer->getTable('sales_order'),
            'trustpayments_security_token',
            [
                'type' => Table::TYPE_TEXT,
                'length' => 32,
                'comment' => 'Trust Payments Payment Security Token'
            ]);

        $installer->getConnection()->addColumn($installer->getTable('sales_order'), 'trustpayments_lock',
            [
                'type' => Table::TYPE_TIMESTAMP,
                'nullable' => true,
                'comment' => 'Trust Payments Payment Lock'
            ]);
    ...
    {
        $installer->getConnection()->addColumn($installer->getTable('quote_payment'), 'trustpayments_token',
            [
                'type' => Table::TYPE_INTEGER,
                'length' => 10,
                'unsigned' => true,
                'comment' => 'Trust Payments Payment Token'
            ]);
    }
    ...
    private function updateInvoiceTable(SchemaSetupInterface $installer)
    {
        $installer->getConnection()->addColumn($installer->getTable('sales_invoice'),
            'trustpayments_capture_pending',
            [
                'type' => Table::TYPE_BOOLEAN,
                'default' => false,
                'comment' => 'Trust Payments Payment Capture Pending'
            ]);
    }
    ...
    private function updateCreditmemoTable(SchemaSetupInterface $installer)
    {
        $installer->getConnection()->addColumn($installer->getTable('sales_creditmemo'),
            ...
            [
                ...
                'length' => 100,
                'nullable' => true,
                'comment' => 'Trust Payments Payment External Id'
            ]);
    }
    ...
    private function createTransactionInfoTable(SchemaSetupInterface $installer)
    {
        if (! $installer->getConnection()->isTableExists(
            $installer->getTable('trustpayments_payment_transaction_info'))) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('trustpayments_payment_transaction_info'))
                ...
                ->addIndex(
                $installer->getIdxName('trustpayments_payment_transaction_info',
                    [
                        'space_id',
                        'transaction_id'
                ...
                ->setComment('Trust Payments Payment Transaction Info');
    ...
    {
        if (! $installer->getConnection()->isTableExists(
            $installer->getTable('trustpayments_payment_method_configuration'))) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('trustpayments_payment_method_configuration'))
                ->addColumn('entity_id', Table::TYPE_INTEGER, null,
                [
                    'identity' => true,
                    'unsigned' => true,
                ...
                ->addIndex(
                $installer->getIdxName('trustpayments_payment_method_configuration', [
                    'space_id'
                ]), [
                    'space_id'
                ])
    ...
    {
        if (! $installer->getConnection()->isTableExists(
            $installer->getTable('trustpayments_payment_refund_job'))) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('trustpayments_payment_refund_job'))
                ->addColumn('entity_id', Table::TYPE_INTEGER, null,
                [
                    'identity' => true,
                    'unsigned' => true,
                ...
                ->addIndex($installer->getIdxName('trustpayments_payment_refund_job', [
                'space_id'
            ]), [
                'space_id'
            ])
                ->addIndex(
                $installer->getIdxName('trustpayments_payment_refund_job', [
                    'order_id'
                ], AdapterInterface::INDEX_TYPE_UNIQUE), [
                    'order_id'
                ], [
    ...
    {
        if (! $installer->getConnection()->isTableExists(
            $installer->getTable('trustpayments_payment_token_info'))) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('trustpayments_payment_token_info'))
                ->addColumn('entity_id', Table::TYPE_INTEGER, null,
                [
                    'identity' => true,
                    'unsigned' => true,
                ...
                ->addIndex($installer->getIdxName('trustpayments_payment_token_info', [
                'customer_id'
            ]), [
                'customer_id'
            ])
                ->addIndex(
                $installer->getIdxName('trustpayments_payment_token_info', [
                    'payment_method_id'
                ]), [
                    'payment_method_id'
                ])
                ...
                ->setComment('Trust Payments Payment Token Info');
            $installer->getConnection()->createTable($table);
        }
    }
}